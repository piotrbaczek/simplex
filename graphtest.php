...
<html>
    <head>
		...
		<script src="http://simplexpg.zz.mu/js/jquery.flot.js"></script>
		<script src="http://simplexpg.zz.mu/js/development-bundle/ui/jquery-ui-1.8.16.custom.js"></script>
		<link rel="stylesheet" href="http://simplexpg.zz.mu/js/css/smoothness/jquery-ui-1.8.16.custom.css"/>
		<style>
			#sliders{
				margin-top: 50px;
				margin-bottom: 50px;
		...
		<script>
			...
			var Grapher = function(count, points2d, points3d, div2d, canvas3d) {
				this.count = count;
			...
			Grapher.prototype.isArray = function(grapher) {
				return Array.isArray(grapher);
			};
			Grapher.prototype.plot3d = function() {
				for (var i = 0; i < this.points3d.length; i++) {
					this.vars.push("Punkt" + (i + 1));
				}
				this.x = {
					"y": {
						"vars": this.vars,
				...
				new CanvasXpress(this.canvas3d.attr('id'), this.x, {
					graphType: "Scatter3D",
					useFlashIE: true,
					xAxis: [
						"X"
			...
			Grapher.prototype.prepare = function() {
				if (this.count === 2) {
					$.plot(this.div1, this.points2d);
				} else {
					this.div1.hide();
					Slider.prototype.hideSlides();
				}
				this.plot3d();
			};
		</script>
		<script>
			$(document).ready(function() {
				...
				var grapher1 = new Grapher(data[0], data[3], data[4], $('#placeholder1'), $('#canvas1'));
